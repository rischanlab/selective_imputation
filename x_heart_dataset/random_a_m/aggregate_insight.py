...
def get_utility_score_topk(k, file):
    my_sheet = 'Sheet1'
    file_name = file  # name of your excel file
    df = read_excel(file_name, sheet_name=my_sheet)
    df.drop(df.columns[[0]], axis=1, inplace=True)
    df = df.drop(df[df.Attributes == 'num'].index)
    df = df.head(k)
    sum_score = df['Utility'].astype(float).sum()
    return sum_score # shows headers with top 5 rows

def get_utility_score_topk_sim(k, file):
    my_sheet = 'Sheet1'
    file_name = file  # name of your excel file
    df = read_excel(file_name, sheet_name=my_sheet)
    df.drop(df.columns[[0]], axis=1, inplace=True)
...
def cum_score(sum_ideal, sum_new):
    #sum_ideal = ideal['score'].astype(float).sum()
    #sum_new = new['score'].astype(float).sum()
    # bound = math.sqrt(2)*k
    # n_sum_ideal = sum_ideal/bound
    # n_sum_new = sum_new/bound

    gap = abs(sum_ideal - sum_new)
