...
table_name = 'heart'
connection = pg.connect("dbname=heart_dataset_random_predicate_cp user=postgres password=zenvisage")
...
engine = create_engine(r'postgresql+psycopg2://postgres:zenvisage@localhost:5432/aheart_dataset_random_predicate_cp')
...
for i in mlist:
    for j in range(100):
        ...
        df_a_m = df_a_m.drop('cp', 1)
        df_a_m_missing = missing_data(df_a_m, size, i, j)
        df_a_m_missing['cp'] = df['cp'].values
